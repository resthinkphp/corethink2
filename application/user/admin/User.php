...
class User extends Admin {
    ...
    public function index() {
        ...
        $builder->setMetaTitle('用户列表') // 设置页面标题
                ...
                ->addTopButton('recycle', array('model' => 'admin_user'))  // 添加删除按钮
        ...
        if (IS_POST) {
            ...
            if ($data) {
                ...
                if ($result) {
                    ...
                    if ($count) {
                        $user_type_name = D('User/Type')->where(array('id' => $type))->getField('name');
                        ...
                        if ($result === false) {
                            $this->error('信息修改失败'.$user_extend_object->getError());
                        } else {
            ...
            if ($attribute_list[$type]) {
                ...
                $new_attribute_list_sort['user']['options']['group_extend']['title'] = $user_type_info['title'].'扩展信息';
